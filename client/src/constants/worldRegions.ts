

export const SCALE_RANGES = {
  weight: ['0-1kg', '1-5kg', '5-20kg', '20-100kg', '100-500kg', '500kg-2t', '2t-10t', '10t+'],
  length: ['0-10cm', '10-50cm', '50cm-1m', '1-2m', '2-5m', '5-10m', '10m+'],
  height: ['0-20cm', '20-50cm', '50cm-1m', '1-2m', '2-3m', '3-5m', '5m+'],
  tailLength: ['No tail', '0-10cm', '10-30cm', '30cm-1m', '1-2m', '2m+']
};

export const PLAYER_COLORS = ['#ef4444', '#3b82f6', '#10b981', '#f59e0b', '#8b5cf6', '#ec4899'];

export const VICTORY_POINTS = {
  2: 120, 3: 120, 4: 100, 5: 100, 6: 80
};